...
var CruncherCtrl = function($scope, $rootScope, $timeout) {
    ...
    var getTypeMultiplierOfUnit = function(attackerType,attackedType, unit, teamSlot) {
        ...
        if ($scope.data.effect == 'Kizuna Clash [Global]'){
            if ([ 3027 ].indexOf(unit.unit.number + 1) != -1) typeMult = getParameters(teamSlot).superType[teamSlot] ? 2.5 : 2;
        }
    ...
    var computeFlatBonusDamage = function(hitModifier, unit, type, teamSlot) {
        ...
        if(staticMultiplier.length >= 1 || sailor){
            ...
            if ($scope.data.effect == 'Kizuna Clash [Global]'){
                if ([ 3027 ].indexOf(unit.number + 1) != -1) affinityMultiplier = affinityMultiplier;
            }
            if ($scope.data.effect == 'Kizuna Clash [Japan]'){
            ...
            if (zombie.type == 'healer') {
                if (zombie.hasOwnProperty('amount')) 
                    ...
                    if (!zombie.multiplier){
                        ...
                        healAmount += (id == 1750 || id == 1751 || id == 1922  || id == 2775  || id == 2776 || id == 5083 || id == 5087 || id == 2959 || id == 2960 || id == 3220 || id == 3221) ? (.5 * (data.team[i].rcv + rcvtemp) * rcvmulttemp * hitsCount['Perfect']) : 0;
